...
public sealed class PostgresDapperPersistenceTests(PostgresDapperFixture fixture)
    : IClassFixture<PostgresDapperFixture>
{
    [Fact]
    ...
    public async Task WriteAsync_WhenTransactionCommits_PersistsOutboxMessage()
    {
        ...
        await PostgresDapperSchema.EnsureDurableMessagingTablesAsync(
            dataSource,
            schema);
        await using var session = new PostgresDapperModuleSession(dataSource);
        var writer = new PostgresDapperDurableOutboxWriter(session, schema: schema);
    ...
    public async Task HandleOnceAsync_WhenHandled_StoresProcessedInbox()
    {
        ...
        await PostgresDapperSchema.EnsureDurableMessagingTablesAsync(
            ...
            schema);
        await using var session = new PostgresDapperModuleSession(dataSource);
        var executor = new PostgresDapperModuleDurableInboxHandlerExecutor(
            "billing",
            session,
            schema: schema);
    ...
    public async Task SaveAsync_WhenStateExists_UpdatesOperationState()
    {
        ...
        await PostgresDapperSchema.EnsureDurableMessagingTablesAsync(
            ...
            schema);
        await using var session = new PostgresDapperModuleSession(dataSource);
        var store = new PostgresDapperDurableOperationStateStore(
            session,
            schema);
