...
public static class PostgresDapperSchema
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(schema))
        {
            await connection.ExecuteAsync(new CommandDefinition(
                $"CREATE SCHEMA IF NOT EXISTS {PostgresDapperTableIdentifier.QuoteIdentifier(schema.Trim())}",
                cancellationToken: ct));
        }

        string outboxTable = PostgresDapperTableIdentifier.Build(
            PostgresDapperDurableTableNames.OutboxMessages,
            schema);
        string inboxTable = PostgresDapperTableIdentifier.Build(
            PostgresDapperDurableTableNames.InboxMessages,
            schema);
        string operationTable = PostgresDapperTableIdentifier.Build(
