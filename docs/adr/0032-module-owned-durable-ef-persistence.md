...
- Current contract: Module-owned durable EF persistence is supported for the
  ...
  inspection and unprocessed inbox inspection resolve the named module's
  inspection store so operator queries use the same module persistence
  boundary as dispatch and receive execution. PostgreSQL provider binding
  should be available from the module builder as the preferred public setup
  shape.
...
- Application evidence: Core module-aware durable persistence contracts and
  ...
  module-specific inbox handler executor, inbox inspection-store, outbox
  dispatcher, and outbox inspection-store bindings are applied. The modular
  monolith adoption-proof
